...
def RetrieveData(Date, args):
    ...
    if TimeStep == 8:
        LSTfileNamePart = os.path.join(output_folder,
                                       'LST_MOD11A2_K_8-daily_' + Date.strftime('%Y') + '.' + Date.strftime(
    ...
    if angle_info == 1:
        OnsangfileNamePart = os.path.join(output_folder,
                                          'Angle_MOD11A1_degrees_daily_' + Date.strftime('%Y') + '.' + Date.strftime(
                                              '%m') + '.' + Date.strftime('%d') + '.tif')
    if time_info == 1:
        TimefileNamePart = os.path.join(output_folder,
                                        'Time_MOD11A1_hour_daily_' + Date.strftime('%Y') + '.' + Date.strftime(
                                            '%m') + '.' + Date.strftime('%d') + '.tif')
    ...
    if not (len(filesMOD) == 1 and len(filesANGLE) == 1 and len(filesTime) == 1):
        ...
        try:
            Collect_data(TilesHorizontal, TilesVertical, Date, output_folder, TimeStep, hdf_library, day_night,
                         angle_info)
        except:
        ...
        try:
            ...
            if TimeStep == 1:
                ...
                if angle_info == 1:
                    ...
                    OnsangfileName = os.path.join(output_folder, 'Angle_MOD11A1_degrees_daily_' + Date.strftime(
...
def Collect_data(TilesHorizontal, TilesVertical, Date, output_folder, TimeStep, hdf_library, day_night, angle_info=0):
    ...
    for Vertical in range(int(TilesVertical[0]), int(TilesVertical[1]) + 1):
        ...
        for Horizontal in range(int(TilesHorizontal[0]), int(TilesHorizontal[1]) + 1):
            ...
            if TimeStep == 8:
                url = 'https://e4ftl01.cr.usgs.gov/MOLT/MOD11A2.061/' + Date.strftime('%Y') + '.' + Date.strftime(
                    '%m') + '.' + Date.strftime('%d') + '/'
            if TimeStep == 1:
                url = 'https://e4ftl01.cr.usgs.gov/MOLT/MOD11A1.061/' + Date.strftime('%Y') + '.' + Date.strftime(
                    '%m') + '.' + Date.strftime('%d') + '/'
