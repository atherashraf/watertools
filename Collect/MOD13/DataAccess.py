...
def RetrieveData(Date, args):
    ...
    NDVIfileName = os.path.join(output_folder, 'NDVI_MOD13Q1_-_16-daily_' + Date.strftime('%Y') + '.' + Date.strftime(
...
def Collect_data(TilesHorizontal, TilesVertical, Date, output_folder, hdf_library):
    ...
    for Vertical in range(int(TilesVertical[0]), int(TilesVertical[1]) + 1):
        ...
        for Horizontal in range(int(TilesHorizontal[0]), int(TilesHorizontal[1]) + 1):
            ...
            url = 'https://e4ftl01.cr.usgs.gov/MOLT/MOD13Q1.061/' + Date.strftime('%Y') + '.' + Date.strftime(
                '%m') + '.' + Date.strftime('%d') + '/'
            ...
            if hdf_library is not None:
                ...
                hdf_name = glob.glob(
                    "MOD13Q1.A%s%03s.h%02dv%02d.*" % (Date.strftime('%Y'), Date.strftime('%j'), Horizontal, Vertical))
            ...
            try:
                ...
                for n in sdslist:
                    ...
                    DataTot[int((countYdata - 1) * 4800):int(countYdata * 4800),
                    int((countX - 1) * 4800):int(countX * 4800)] = data * 0.0001
                del data
