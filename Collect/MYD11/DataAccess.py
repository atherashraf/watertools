...
def DownloadData(Dir, Startdate, Enddate, latlim, lonlim, TimeStep, Waitbar, cores, hdf_library, remove_hdf,
                 angle_info=0, time_info=0, day_night="day"):
    """
    ...
    TilesVertical, TilesHorizontal = watertools.Collect.MOD15.DataAccess.Get_tiles_from_txt(output_folder, hdf_library,
                                                                                            latlim, lonlim)
...
def RetrieveData(Date, args):
    ...
    # Argument
    [output_folder, TilesVertical, TilesHorizontal, lonlim, latlim, TimeStep, hdf_library, angle_info, time_info,
     day_night] = args

    # Define output names
    if TimeStep == 8:
        LSTfileNamePart = os.path.join(output_folder,
                                       'LST_MYD11A2_K_8-daily_' + Date.strftime('%Y') + '.' + Date.strftime(
                                           '%m') + '.' + Date.strftime('%d') + '.tif')
    if TimeStep == 1:
        LSTfileNamePart = os.path.join(output_folder,
                                       'LST_MYD11A1_K_daily_' + Date.strftime('%Y') + '.' + Date.strftime(
                                           '%m') + '.' + Date.strftime('%d') + '*.tif')
    filesMOD = glob.glob(LSTfileNamePart)

    if angle_info == 1:
        OnsangfileNamePart = os.path.join(output_folder,
                                          'Angle_MYD11A1_degrees_daily_' + Date.strftime('%Y') + '.' + Date.strftime(
                                              '%m') + '.' + Date.strftime('%d') + '.tif')
    if angle_info == 1:
    ...
    if time_info == 1:
        TimefileNamePart = os.path.join(output_folder,
                                        'Time_MYD11A1_hour_daily_' + Date.strftime('%Y') + '.' + Date.strftime(
                                            '%m') + '.' + Date.strftime('%d') + '.tif')
    ...
    if not (len(filesMOD) == 1 and len(filesANGLE) == 1 and len(filesTime) == 1):
        ...
        try:
            Collect_data(TilesHorizontal, TilesVertical, Date, username, password, output_folder, TimeStep, hdf_library,
                         day_night, angle_info)
        except:
        ...
        try:
            ...
            if TimeStep == 1:
                ...
                if time_info == 1:
                    TimefileName = os.path.join(output_folder,
                                                'Time_MYD11A1_hour_daily_' + Date.strftime('%Y') + '.' + Date.strftime(
                                                    '%m') + '.' + Date.strftime('%d') + '.tif')
                ...
                if angle_info == 1:
                    ...
                    OnsangfileName = os.path.join(output_folder, 'Angle_MYD11A1_degrees_daily_' + Date.strftime(
                        '%Y') + '.' + Date.strftime('%m') + '.' + Date.strftime('%d') + '.tif')
...
def Collect_data(TilesHorizontal, TilesVertical, Date, username, password, output_folder, TimeStep, hdf_library,
                 day_night, angle_info=0):
    '''
    ...
    for Vertical in range(int(TilesVertical[0]), int(TilesVertical[1]) + 1):
        ...
        for Horizontal in range(int(TilesHorizontal[0]), int(TilesHorizontal[1]) + 1):
            ...
            if TimeStep == 8:
                url = 'https://e4ftl01.cr.usgs.gov/MOLA/MYD11A2.061/' + Date.strftime('%Y') + '.' + Date.strftime(
                    '%m') + '.' + Date.strftime('%d') + '/'
            if TimeStep == 1:
                url = 'https://e4ftl01.cr.usgs.gov/MOLA/MYD11A1.061/' + Date.strftime('%Y') + '.' + Date.strftime(
                    '%m') + '.' + Date.strftime('%d') + '/'
            ...
            if hdf_library is not None:
                ...
                if TimeStep == 8:
                    hdf_name = glob.glob("MYD11A2.A%s%03s.h%02dv%02d.*" % (
                    Date.strftime('%Y'), Date.strftime('%j'), Horizontal, Vertical))
                if TimeStep == 1:
                    hdf_name = glob.glob("MYD11A1.A%s%03s.h%02dv%02d.*" % (
                    Date.strftime('%Y'), Date.strftime('%j'), Horizontal, Vertical))
    ...
    if TimeStep == 1:
        ...
        driver = gdal.GetDriverByName("GTiff")
        dst_ds = driver.Create(name_out_time, DataTot_Time.shape[1], DataTot_Time.shape[0], 1, gdal.GDT_Float32,
                               ['COMPRESS=LZW'])
        try:
            dst_ds.SetProjection(proj)
        except:
    ...
    if angle_info == 1:
        ...
        driver = gdal.GetDriverByName("GTiff")
        dst_ds = driver.Create(name_out_obsang, DataTot_ObsAng.shape[1], DataTot_ObsAng.shape[0], 1, gdal.GDT_Float32,
                               ['COMPRESS=LZW'])
        try:
            dst_ds.SetProjection(proj)
        except:
...
def Tiles_to_download(tiletext2, lonlim1, latlim1):
    ...
    latlimtiles1UL = tiletextExtremes[
        np.logical_and(tiletextExtremes[:, 2] <= latlim1[1], tiletextExtremes[:, 3] >= latlim1[1])]
    latlimtilesUL = latlimtiles1UL[
        np.logical_and(latlimtiles1UL[:, 4] <= lonlim1[0], latlimtiles1UL[:, 5] >= lonlim1[0])]

    # Define the lower right tile
    latlimtiles1LR = tiletextExtremes[
        np.logical_and(tiletextExtremes[:, 2] <= latlim1[0], tiletextExtremes[:, 3] >= latlim1[0])]
    latlimtilesLR = latlimtiles1LR[
        np.logical_and(latlimtiles1LR[:, 4] <= lonlim1[1], latlimtiles1LR[:, 5] >= lonlim1[1])]
