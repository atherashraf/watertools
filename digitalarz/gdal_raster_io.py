...
class GDALRasterIO:
    ...
    @classmethod
    def write_geotiff(cls, filename, arr, bands=1, proj_crs: pyproj.CRS = None, geo_transform: np.array = None):

        arr_type = gdal.GDT_Float32 if arr.dtype == np.float32 else gdal.GDT_Int32

        driver = gdal.GetDriverByName("GTiff")
        out_ds = driver.Create(filename, arr.shape[1], arr.shape[0], bands, arr_type)
        if proj_crs is not None:
            # proj = cls.crs_from_pyproj(proj_crs)
    ...
    @classmethod
    def from_numpy(cls, data, geo_transform, proj_crs, bands=1):
        arr_type = gdal.GDT_Float32 if data.dtype == np.float32 else gdal.GDT_Int32
        driver = gdal.GetDriverByName("MEM")
        out_ds = driver.Create("in-mem-raster", data.shape[1], data.shape[0], bands, arr_type)
        if geo_transform is not None:
